...
TEST_F(EndToEndPerformanceTest, SingleCommandOverhead) {
  ...
  std::cout << "Total time for " << num_calls << " calls: " 
...
TEST_F(EndToEndPerformanceTest, MaxConcurrentInstruments) {
  ...
  for (int i = 2; i <= max_instruments; i++) {
    std::string config = R"(
name: MockInstrument)" + std::to_string(i) + R"(
api_ref: tests/data/mock_api.yaml
connection:
  type: VISA
  address: "mock://test)" + std::to_string(i) + R"("
)";
    
    std::string config_path = "/tmp/mock_instrument_" + std::to_string(i) + ".yaml";
  ...
  std::cout << "Execution time for " << (calls_per_instrument * (instrument_names.size() + 1)) 
            << " calls: " << exec_duration.count() << " ms\n";
  std::cout << "Average latency per call: " 
            << (exec_duration.count() * 1000.0) / (calls_per_instrument * (instrument_names.size() + 1))
            << " µs\n";
  
  // Clean up
  for (size_t i = 0; i < instrument_names.size(); i++) {
    registry.remove_instrument(instrument_names[i]);
    std::remove(("/tmp/mock_instrument_" + std::to_string(i + 2) + ".yaml").c_str());
  }
}
...
connection:
  ...
  std::cout << "Average latency per parallel block (2 commands): " << avg_latency << " µs\n";
  std::cout << "Throughput: " << (num_parallel_blocks * 1000000.0) / duration.count() 
            << " parallel blocks/sec\n";
  std::cout << "Total time for " << num_parallel_blocks << " parallel blocks: " 
            << duration.count() / 1000.0 << " ms\n";
