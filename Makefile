BUILD_DIR=./build
build:
	mkdir -p build && cd build && CC="ccache clang" CXX="ccache clang++" cmake -G Ninja -DCMAKE_C_FLAGS="--coverage" -DCMAKE_CXX_FLAGS="--coverage -Oz -g" -DCMAKE_EXE_LINKER_FLAGS="-fuse-ld=lld" -DCMAKE_EXPORT_COMPILE_COMMANDS=ON .. && ninja
clean:
	rm -rf build
